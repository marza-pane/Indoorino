...
SOURCEPATH = os.path.dirname(os.path.abspath(__file__)).replace('application/utils', '')
...
# rm -r build/ && clc && python3.7 setup.py build && cp build/lib.linux-x86_64-3.7/Indoorino.cpython-37m-x86_64-linux-gnu.so Indoorino.so
...
def build_indoorino_module():
    ...
    path_target = os.path.join(SOURCEPATH, 'application/utils/IndoorinoModule.so')
